...
color WHITE = #FFFFFF; //NetheriteBlocks
color BLACK = #000000; //EmptySpace
color YELLOW = #ffca18; //GoldBlock
color ORANGE = #ff7f27; // ShroomLight
color LIGHTBLUE = #7092be; //Glass
...
void setup() {
  ...
  quartz = loadImage("SmoothQuartz.png");
...
void drawFloor(int start, int end, int level, int gap) {
  ...
  int z = start;
  while (z < end) {
    texturedBlock(x, level, z, quartz, gap);
    x += gap;
    if (x >= end) {
      z += gap;
      x = start;
  ...
  for (int x = 0; x < map.width; x ++) {
    for (int y = 0; y < map.height; y ++) {
      ...
      for (int tempHeight = 1; tempHeight < mapHeight; tempHeight ++) {
        if (c == YELLOW) {
          texturedBlock(x*gridSize-2000, height-gridSize*tempHeight, y*gridSize-2000, goldblock, gridSize);
        
        }
        if (c == ORANGE) {
          texturedBlock(x*gridSize-2000, height-gridSize*tempHeight, y*gridSize-2000, shroomlight, gridSize);
         
        }
        if (c == BLACK) {
          texturedBlock(x*gridSize-2000, height-gridSize*tempHeight, y*gridSize-2000, netheriteblock, gridSize);
       
        }
        if (c == LIGHTBLUE) {
          texturedBlock(x*gridSize-2000, height-gridSize*tempHeight, y*gridSize-2000, glass, gridSize);
